...
class ScoreBar extends Component {
  render() {
    const { user1Score, user2Score, value } = this.props;
    const user1ScoreParse = parseInt(user1Score);
    const user2ScoreParse = parseInt(user2Score);
    return (
      <ScoreBarContainer>
        <ScoreBarNumbers>
        <ScoreBarScore1>{user1ScoreParse}<br/> <span>{value}</span></ScoreBarScore1>
        <ScoreBarScore2>{user2ScoreParse}<br/> <span>{value}</span></ScoreBarScore2>
        </ScoreBarNumbers>
